...
def plot_head_tail_positions(x_head_path: list, y_head_path: list, x_tail_path: list, y_tail_path: list) -> None:
...
def moves(data: pd.DataFrame) -> list:
    ...
    for row in data:
        ...
        for _ in range(0,steps):
            ...
            distance = np.sqrt(np.power(x_head - x_tail, 2) + np.power(y_head - y_tail, 2))
            if distance >= 2: # move tail
                [x_tail, y_tail] = [x_tail + np.sign(x_head - x_tail), y_tail + np.sign(y_head - y_tail)]
...
def main(visualize: bool = typer.Argument(False)):
    """Solve part 1."""
    data = read_split_data("days/data/input9.txt")
