...
class Directory:
    ...
    def get_total_size(self, directories: dict) -> int:   
        ...
        for directory in directories:
            if self.name in directories[directory].name and not self.name == directories[directory].name:
    ...
    for line in puzzle_input[1:]:
        ...
        if "$ cd" in line:
            ...
            # Go to child directory
                ...
                new_directory_name = f"{name}/{sub_directory_name}" if name!="/" else f"{name}{sub_directory_name}"
...
def smallest_size_directory_to_delete(directories: dict[Directory], size_filesystem: int, free_space_needed: int) -> int:
